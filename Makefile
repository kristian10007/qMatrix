...
testResults/%/brute-force.ok: testDataSets/%.csv
	...
	mkdir -p "$p"
	$(eval outParam := "-o=$p/qMatrix.csv" "-oi=$p/projection.pdf" "-op=$p/projection.csv" "-od=$p/dendrogram.pdf" "-ou=$p/umap.pdf" "-omnb=$p/ModularityImage.pdf" "-layer2=$p/layer2_prediction.csv" "-functions=$p/functions.csv")
...
fastTest:
	# ===[ qMatrix ]==========================================================
	python3 qMatrix.py -i=patient_ids -i=id -o=- testDataSets/test_v0.csv -layer2=- -functions=-
	#
	# ===[ qMatrix via tree (original version) ]==============================
	python3 qMatrix.py -i=patient_ids -i=id -tree -o=- testDataSets/test_v0.csv -layer2=- -functions=-
	#
	# ===[ qMatrix via tree (fast version) ]==================================
	python3 qMatrix.py -i=patient_ids -i=id -tree-fast -o=- testDataSets/test_v0.csv -layer2=- -functions=-
