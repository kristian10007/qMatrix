...
testResults/%/brute-force.ok: testDataSets/%.csv
	...
	mkdir -p "$p"
	$(eval outParam := "-o=$p/qMatrix.csv" "-oi=$p/projection.pdf" "-op=$p/projection.csv" "-od=$p/dendrogram.pdf" "-ou=$p/umap.pdf" "-omnb=$p/ModularityImage.pdf" "-layer2=$p/layer2_prediction.csv")
