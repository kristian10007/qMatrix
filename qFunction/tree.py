...
def build_feature_tree_with_progress(data, n_jobs=-1):
...
def knn_features(tree, query_feature, data, k=5):
...
def compute_knn_dependency_matrix(tree, data, k=5):
    ...
    for query_feature in tqdm(range(n_features), desc="Computing kNN for all features"):
        neighbors_with_scores = knn_features(tree, query_feature=query_feature, data=data, k=k)
